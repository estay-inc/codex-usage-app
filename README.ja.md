...
Codexの週次枠の残量、今日の消費量、直近1時間の消費量をmacOSのメニューバーへ
...
- メニューバーに週次枠の残量（`W`）、今日の消費量（`1D`）、直近1時間の
  消費量（`1H`）を表示
...
- `W 82%`は、週次枠が82%残っていることを示します。
- `1D 6%`は、Macの現地時刻で今日0時から週次枠を6%使用したことを示します。
- `1H 2%`は、直近1時間で週次枠を2%使用したことを示します。
- 更新初日に0時の基準値がない場合や、週次リセットを日中にまたいだ場合は
  `1D …`、最初の1時間は`1H …`と表示します。
- 通常は`W 82%  1D 6%  1H 2%`、5時間枠も取得できる場合は
  `5h 70%  W 82%  1D 6%  1H 2%`のように表示します。
