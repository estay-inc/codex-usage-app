...
pace_output="$("$ROOT_DIR/build/Codex Usage.app/Contents/MacOS/CodexUsageMenuBar" --pace-test)"
...
grep -Fq 'total_limit=Total limit' <<<"$english_output"
grep -Fq 'today_partial=Today: Used at least 3%' <<<"$english_output"
...
grep -Fq 'total_limit=トータル枠' <<<"$japanese_output"
grep -Fq 'today_partial=今日: 使用 3%以上（計測開始後）' <<<"$japanese_output"
grep -Fq 'quit=終了' <<<"$japanese_output"
grep -Fq 'OK rate-limit-window-classification' <<<"$parser_output"
grep -Fq 'OK pace-classification-and-coloring' <<<"$pace_output"
